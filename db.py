...
class ProblemsDataBase():
    ...
    def __init__(self) -> None:
        self.connect = sqlite3.connect('problems.sqlite')
    ...
    def get_one_problem_by_tag(self, tag_id) -> Problem:
        ...
        connect = sqlite3.connect('problems.sqlite')
        cursor = connect.cursor()
        cursor.execute("SELECT * FROM Problems p JOIN Problems_Tags pt ON p.problemId = pt.problemId WHERE pt.tagId =?",
                       (tag_id,))
    ...
    def addProblem(self, problemId, problem_index, name, problem_type, points, rating, tags: list):
        ...
        try:
            cursor.execute(
                f"INSERT INTO Problems (problemId, problem_index, name, type, points, rating) VALUES ('{problemId}', '{problem_index}', '{name}', '{problem_type}', {points}, {rating})")
        except:
            pass
        print(tags)
        for tag in idTags.keys():
            if tag in tags:
                cursor.execute(
                    F"INSERT INTO Problems_Tags (tagId, problemId) VALUES ({idTags.get(tag)}, '{problemId}')")
